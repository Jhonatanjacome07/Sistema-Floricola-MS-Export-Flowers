...
<div id="header-section" style="display: none; padding: 20px; background-color: #f8f9fa; font-family: 'Times New Roman';">
    ...
    <p style="margin-bottom: 5px; display: inline-block; font-family: 'Times New Roman';" id="plaga-label"><strong>Plagas:</strong></p>
    <p style="margin-bottom: 5px; font-family: 'Times New Roman';">
        <strong>Elaborado por: </strong>{{ Auth::user()->name }} {{ Auth::user()->lastname }} encargado de la 
...
<form method="GET" action="{{ route('flornacionalreporteparametros') }} " id="report-form">
    <div class="row" id="input-section">
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="plaga" id="plaga" class="form-control" onchange="this.form.submit()">
                    <option value="">Todas las plagas</option>
                    @foreach($plagas as $plaga)
                    <option value="{{ $plaga }}" {{ request()->input('plaga') == $plaga ? 'selected' : '' }}>
                        {{ $plaga }}
                    </option>
                    @endforeach
                </select>
...
<script>
    ...
    document.getElementById("end-date").innerHTML = "<strong>Fecha de finalización:</strong> " + formattedEndDate;
...
<script>
    ...
    function printReport() {
        ...
        document.getElementById('end-date').textContent = 'Fecha de finalización: ' + endDate;
...
<script>
    // Obtener el valor seleccionado de la plaga
    var selectedPlaga = document.getElementById("plaga").value;

    // Otra forma de obtener el valor seleccionado de la plaga usando el nombre del select
    var selectedPlaga = document.querySelector("select[name='plaga']").value;

    // Obtener el elemento de párrafo para mostrar la plaga seleccionada
    var plagaParagraph = document.getElementById("plaga-label");

    // Mostrar la plaga seleccionada en el párrafo
    if (selectedPlaga === "") {
        plagaParagraph.innerHTML = "<strong>Plaga:</strong> Todas las plagas";
    } else {
        plagaParagraph.innerHTML = "<strong>Plaga:</strong> " + selectedPlaga;
    }
</script>
...
<style>
    @media print {
        ...
        /* Estilo para la línea debajo de la firma */
        #signature-row td {
            padding-bottom: 30px; /* Ajusta el espacio entre la firma y la línea */
             border-top: 3px solid black;
            width: 20%;
            margin: 0 auto;
        }
         /* Estilos para la línea de la firma */
        .signature-line {
            border-top: 3px solid black;
            width: 20%;
            margin: 0 auto;
