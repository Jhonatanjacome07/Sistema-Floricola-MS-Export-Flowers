...
@section('content_header')
    <h1 class="text-center"> <b>Editar Plaga</b></h1>
@stop

@section('content')

    <section>
        <form method="POST" action="{{ route('plagas.update', $plagas) }}" class="mx-auto" style="max-width: 950px;">
            @method('PUT')
            @csrf
                    ...
                    <div class="row">
                        <div class="col-md-6" >
                            <x-adminlte-input type="text" name="name" label="Nombre de la plaga" label-class="text-lightblue"
                                placeholder="Ingrese el nombre" value="{{ $plagas->name }}">
                                @error('name')
